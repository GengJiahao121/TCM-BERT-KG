...
def evaluate(args, model, symptoms_kg_embeddings, herb_kg_embeddings, tokenizer, prefix=""):
    ...
    with open(output_eval_file, "w") as writer:
        ...
        #     logger.info("  %s = %s", key, str(result[key]))
            # writer.write("%s = %s\n" % (key, str(result[key])))
        logger.info("  %s = %s\n", "accuracy", str(result['acc']))
        logger.info("  %s = %s\n", "precision", str(result['precision']))
        logger.info("  %s = %s\n", "recall", str(result['recall']))
        logger.info("  %s = %s\n", "f1", str(result['f1']))
        logger.info("  %s = %s\n", "top_k_accuracy_value", str(result['top_k_accuracy_value']))
...
def main():
    ...
    parser.add_argument('--kg_best_model', type=str, default='/root/autodl-tmp/bert_kg/kg-embeddings/best_valid.model')
    parser.add_argument('--sympotm_entities_ids', type=str, default='/root/autodl-tmp/bert_kg/kg-embeddings/src_data/custom_graph/entity_relation/症状_entity.txt')
    parser.add_argument('--herb_entities_ids', type=str, default='/root/autodl-tmp/bert_kg/kg-embeddings/src_data/custom_graph/entity_relation/中药_entity.txt')
