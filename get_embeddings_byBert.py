...
if __name__ == '__main__':
    ...
    tcm_bert_tokenizer = tokenizer_class.from_pretrained(tokenizer_name if tokenizer_name else model_name_or_path,
                                                ...
                                                cache_dir=cache_dir if cache_dir else None)
    tcm_bert_model = model_class.from_pretrained(model_name_or_path,
    ...
    for herb in herb_list:
        input = tcm_bert_tokenizer.encode_plus(herb, add_special_tokens=True, max_length=max_herb_len)
    ...
    attention_mask_list = torch.tensor(attention_mask_list).to('cuda')
    torch.save(attention_mask_list, '/root/autodl-tmp/get_herb_embeddings/hidden_states_attention_mask_list')

    
    all_cls = []
    all_hidden_states = []
    for input in tqdm(inputs, desc="processing>>>"):
        output = tcm_bert_model(**input)
        cls = output[0]
    ...
    torch.save(all_cls, '/root/autodl-tmp/get_herb_embeddings/output/cls_embeddings')


    all_hidden_states = torch.cat(all_hidden_states, dim=0)
    torch.save(all_hidden_states, '/root/autodl-tmp/get_herb_embeddings/output/hidden_states_embeddings')
